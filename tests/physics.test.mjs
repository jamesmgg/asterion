...
const earth = { mass: 5.9722e24, radius: 6371000, atmosphere: { density: 1.225, height: 8500 }, kind: 'rocky', targetDensity: 2750 };
const moon = { mass: 7.342e22, radius: 1737400, atmosphere: null, kind: 'rocky', targetDensity: 2750 };
...
test('Kepler solver reaches a small residual even for eccentric orbits', () => {
  assert.equal(typeof p.solveKepler, 'function', 'Kepler solver is implemented');
  for (const e of [0, .0167, .206, .9]) for (const m of [-3, 0, .5, 2.8]) {
    const E = p.solveKepler(m, e); assert.ok(Math.abs(E - e * Math.sin(E) - m) < 1e-10);
  }
});
test('Earth at J2000 agrees with published approximate heliocentric coordinates', () => {
  ...
  assert.ok(Math.abs(pos[0] - (-.17717)) < .0001);
  assert.ok(Math.abs(pos[1] - .96721) < .0001);
  assert.ok(Math.abs(pos[2]) < .00001);
});
test('Nine-body solar system conserves energy and momentum over a simulated year', () => {
  ...
  const start = sim.energy(); const initialMomentum = sim.momentum();
  ...
  assert.ok(sim.bodies.every(b => [...b.position, ...b.velocity].every(Number.isFinite)));
});
test('A paused orbit does not advance', () => {
  assert.equal(typeof p.SolarSystem, 'function');
  const sim = new p.SolarSystem(2461293); const before = JSON.stringify(sim.bodies);
  sim.advance(0); assert.equal(JSON.stringify(sim.bodies), before);
});
...
test('Airless impacts conserve mechanical energy through gravitational acceleration', () => {
  ...
  const expected = Math.sqrt(20000 ** 2 + 2 * 6.67430e-11 * moon.mass * (1 / moon.radius - 1 / (moon.radius + result.entryAltitude)));
  assert.ok(Math.abs(result.surfaceSpeed - expected) / expected < .005);
...
test('Small stony Earth impactors deposit energy in the atmosphere', () => {
  assert.equal(typeof p.calculateImpact, 'function');
  const result = p.calculateImpact(earth, {...impact, diameter: 20, speed: 19});
  ...
  assert.ok(result.atmosphereEnergy > result.entryEnergy * .5);
});
...
test('Gas giants never report a solid surface crater', () => {
  assert.equal(typeof p.calculateImpact, 'function');
  const result = p.calculateImpact({...earth, mass: 1.898e27, radius: 69911000, kind:'gas', atmosphere:{density:.16,height:27000}}, impact);
...
test('Invalid impact inputs are rejected instead of returning misleading numbers', () => {
  assert.equal(typeof p.calculateImpact, 'function');
  for (const bad of [{diameter:0},{speed:NaN},{angle:0},{angle:100},{material:'wood'}]) {
